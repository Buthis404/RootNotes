...
_NODE_KNOWN = {
    "id", "host_id", "x", "y", "label", "ip", "ips", "type", "status", "ports",
    "notes", "role", "os", "tags", "is_attacker", "manually_positioned",
    "auto_positioned", "updated_at", "version",
}
_EDGE_KNOWN = {
    "id", "from", "to", "from_node_id", "to_node_id", "style", "type", "label",
    "confidence", "source", "reason", "state", "verified", "is_manual",
    "manual_override", "updated_at", "version",
}


def _node_extra(n: dict):
    extra = {k: v for k, v in n.items() if k not in _NODE_KNOWN}
    return json.dumps(extra) if extra else None


def _edge_extra(e: dict):
    extra = {k: v for k, v in e.items() if k not in _EDGE_KNOWN}
    return json.dumps(extra) if extra else None


def _migrate_node(conn, net_id: str, pid: str, n: dict) -> None:
    nid = n.get("id")
    if not nid:
        return
    conn.execute(
    ...
    """),
        {
            "id": nid, "nid": net_id, "pid": pid, "host_id": n.get("host_id"),
            "x": float(n.get("x") or 0), "y": float(n.get("y") or 0),
            "label": n.get("label") or "", "ip": n.get("ip") or "",
            "ips": n.get("ips") or [], "type": n.get("type") or "host",
            "status": n.get("status") or "unknown",
            "ports": json.dumps(n.get("ports") or []),
            "notes": n.get("notes") or "", "role": n.get("role") or "",
            "os": n.get("os") or "", "tags": n.get("tags") or [],
            ...
            "extra_json": _node_extra(n),
        },
    )


def _migrate_edge(conn, net_id: str, pid: str, e: dict) -> None:
    eid = e.get("id")
    if not eid:
        return
    conn.execute(
    ...
    """),
        {
            ...
            "style": e.get("style") or "solid", "type": e.get("type") or "network",
            "label": e.get("label") or "",
            "confidence": int(e.get("confidence") or 100),
            "source": e.get("source") or "manual",
            "reason": e.get("reason") or "", "state": e.get("state") or "manual",
            ...
            "extra_json": _edge_extra(e),
        },
    )


def _migrate_region(conn, net_id: str, pid: str, r: dict) -> None:
    rid = r.get("id")
    if not rid:
        return
    conn.execute(
    ...
    """),
        {
            ...
            "w": float(r.get("w") or 200), "h": float(r.get("h") or 100),
            "label": r.get("label") or "", "note": r.get("note") or "",
            "fill": r.get("fill") or "", "stroke": r.get("stroke") or "",
...
def upgrade():
    ...
    networks = conn.execute(
        sa.text("SELECT id, pid, nodes_json, edges_json, regions_json FROM networks")
    ).fetchall()

    for net_id, pid, nodes_raw, edges_raw, regions_raw in networks:
        for n in _parse(nodes_raw):
            _migrate_node(conn, net_id, pid, n)
        for e in _parse(edges_raw):
            _migrate_edge(conn, net_id, pid, e)
        for r in _parse(regions_raw):
            _migrate_region(conn, net_id, pid, r)
...
def downgrade():
    ...
    conn.execute(
        sa.text(
            "ALTER TABLE networks ADD COLUMN IF NOT EXISTS nodes_json JSONB NOT NULL DEFAULT '[]'"
        )
    )
    conn.execute(
        sa.text(
            "ALTER TABLE networks ADD COLUMN IF NOT EXISTS edges_json JSONB NOT NULL DEFAULT '[]'"
        )
    )
    conn.execute(
        sa.text(
            "ALTER TABLE networks ADD COLUMN IF NOT EXISTS regions_json JSONB NOT NULL DEFAULT '[]'"
        )
    )
