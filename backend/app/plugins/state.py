...
def list_attacker_targets_for_exec(db: Session | None = None) -> list[dict]:
    ...
    return [
        t
        for t in list_attacker_targets(db)
...
def list_attacker_targets_for_pivot(db: Session | None = None) -> list[dict]:
    ...
    return [
        t for t in list_attacker_targets(db) if t.get("enabled", True) and t.get("runs_pivot", True)
    ]
...
def _register_custom_modules(custom_modules: list) -> None:
...
def apply_saved_state():
    ...
    _register_custom_modules(state.get("custom_modules", []))
...
def list_modules() -> list[dict]:
    return [
        ...
        for m in sorted(
            registry.get_all(),
            key=lambda mod: (mod.source != "builtin", (mod.title or mod.name).lower()),
        )
    ]


def create_custom_module(
    name: str, title: str, version: str, description: str, enabled: bool = True
) -> dict:
...
def _update_custom_module_item(item: dict, module, enabled) -> None:
...
def update_module(
    name: str, *, enabled=None, title=None, version=None, description=None
) -> dict | None:
    module = registry.get(name)
    if not module:
        return None
    ...
    if module.source == "custom":
        for item in state.get("custom_modules", []):
            if item.get("name") == name:
                _update_custom_module_item(item, module, enabled)
                break
...
def delete_custom_module(name: str) -> bool:
    ...
    state["custom_modules"] = [
        item for item in state.get("custom_modules", []) if item.get("name") != name
    ]
