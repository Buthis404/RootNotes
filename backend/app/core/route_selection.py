...
def _extract_target_networks(target_hint: str) -> list[ipaddress._BaseNetwork]:
    ...
    if "://" in text:
        try:
            ...
            logger.debug("could not parse URL hostname from target hint %r: %s", text, e)
...
def annotate_targets_with_route_context(
    pid: str, targets: list[dict], db: Session, target_hint: str = ""
) -> list[dict]:
    active = (
        db.query(models.PivotObservation)
        .filter(models.PivotObservation.pid == pid, models.PivotObservation.status == "active")
        .all()
    )
    ...
    annotated.sort(
        key=lambda item: (
            -int(item.get("route_match_score") or 0),
