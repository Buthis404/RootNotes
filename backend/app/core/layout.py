...
_GATEWAY_TAGS = {"gateway", "router", "firewall", "fw", "pivot", "perimeter", "border"}
_GATEWAY_OS = (
    "cisco",
    "juniper",
    "pfsense",
...
def _count_private_ips(hosts: list) -> int:
    count = 0
    for h in hosts:
        try:
...
def _subnet_depth(subnet_hosts: list) -> int:
    ...
    if subnet_hosts and _count_private_ips(subnet_hosts) == 0:
        return 0
...
def _build_subnet_adjacency(
    existing_nodes: list, existing_edges: list
) -> tuple[dict, dict]:
    """Build ip→subnet and subnet→adjacent-subnets mappings from graph data."""
    ip_to_sub: dict = {}
...
def _barycenter_reorder(
    ...
    """
    _ip_to_sub, adj = _build_subnet_adjacency(existing_nodes, existing_edges)
    ...
    for depth in sorted_depths[1:]:
        ...
        layers[depth] = sorted(layer, key=lambda s, b=bary: (b.get(s, float("inf")), s))
        for i, s in enumerate(layers[depth]):
            sub_position[s] = float(i)


def _host_sort_key(h: dict) -> tuple:
    return (-len(h.get("ports") or []), h.get("ip") or "")


def _collect_manual_positions(existing_nodes: list) -> dict:
    manual: dict = {}
    for node in existing_nodes:
...
def _place_row_nodes(row_slice: list, row_x0: int, y_cursor_t: int, row_idx: int, manual: dict, result: list) -> None:
...
def _place_cluster_nodes(tiers: dict, ox: int, oy: int, cw: int, manual: dict) -> list:
    result: list = []
    y_cursor_t = oy + CLUSTER_PAD_Y
    for tier in sorted(tiers.keys()):
        tier_hosts = sorted(tiers[tier], key=_host_sort_key)
        rows = math.ceil(len(tier_hosts) / NODES_PER_ROW)
        for row_idx in range(rows):
            row_slice = tier_hosts[row_idx * NODES_PER_ROW : (row_idx + 1) * NODES_PER_ROW]
            row_w = len(row_slice) * NODE_W
            row_x0 = ox + CLUSTER_PAD_X + max(0, (cw - 2 * CLUSTER_PAD_X - row_w) // 2)
            _place_row_nodes(row_slice, row_x0, y_cursor_t, row_idx, manual, result)
        y_cursor_t += rows * NODE_H + TIER_GAP
    return result


def _place_attackers(attackers: list, cluster_origin: dict, manual: dict) -> list:
    result: list = []
    if not attackers:
        return result
...
def _assign_layers(subnet_tiers: dict, existing_edges: list, existing_nodes: list) -> dict:
    """Compute depth-bucketed layers from subnet_tiers, with optional barycenter reorder."""
    depth_of: dict = {}
...
def _compute_cluster_origins(layers: dict, sorted_depths: list, cluster_dims: dict) -> dict:
    """Compute top-left (x, y) origin for each subnet cluster."""
    layer_y: dict = {}
    y_cursor = CANVAS_TOP
    for depth in sorted_depths:
        max_h = max((cluster_dims[s][1] for s in layers[depth]), default=0)
...
def compute_layout(
    ...
    manual = _collect_manual_positions(existing_nodes) if keep_manual else {}
    ...
    # Phase 1: group by subnet+tier
    subnet_tiers: dict = defaultdict(lambda: defaultdict(list))
    for h in regular:
        subnet_tiers[_host_subnet(h)][_device_tier(h)].append(h)

    # Phase 2-3: assign layers and compute origins
    layers = _assign_layers(subnet_tiers, existing_edges, existing_nodes)
    cluster_dims: dict = {s: _cluster_dims(t) for s, t in subnet_tiers.items()}
    cluster_origin = _compute_cluster_origins(layers, sorted(layers.keys()), cluster_dims)
    ...
    for subnet, tiers in subnet_tiers.items():
        ...
        result.extend(_place_cluster_nodes(tiers, ox, oy, cw, manual))

    result.extend(_place_attackers(attackers, cluster_origin, manual))
    return result
