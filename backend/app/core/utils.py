...
from .. import models
from .config import DEFAULT_CATALOG_PATH, SAFE_UPLOAD_RE, UPLOAD_ROOT
...
def stable_edge_id(from_nid: str, to_nid: str, source: str, kind: str = "") -> str:
    ...
    """
    raw = f"{from_nid or ''}|{to_nid or ''}|{source or ''}|{kind or ''}".encode()
    return "edg" + hashlib.sha256(raw).hexdigest()[:12]
...
def infer_scope_type(value: str) -> str:
    ...
    if urlsplit(val).scheme in {"http", "https"}:
        return "url"
    ...
    if "." in val and not re.match(r"^\d{1,3}(?:\.\d{1,3}){3}$", val):
        return (
            "domain"
            if val.count(".") == 1
            or val.endswith(".local")
            or val.endswith(".corp")
            or val.endswith(".lan")
            else "hostname"
        )
    return "hostname"
...
def sync_project_ip_from_scopes(db: Session, pid: str):
    ...
    if not project:
        return
    scopes = db.query(models.Scope).filter(models.Scope.pid == pid, models.Scope.in_scope).all()
    values = [
        s.value.strip()
        for s in scopes
...
def sync_scopes_from_project_ip(db: Session, pid: str):
    ...
    scopes = (
        ...
        .filter(models.Scope.pid == pid, models.Scope.in_scope, models.Scope.scope_type == "cidr")
        .all()
    )
