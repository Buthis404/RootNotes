...
def _upsert_host_fallback(
    db: Session, pid: str, ip: str, defaults: dict, update_on_conflict: dict | None
    ...
    host = models.Host(
        id=defaults.get("id") or new_id("hst"),
        pid=pid,
        ip=ip,
        **{k: v for k, v in defaults.items() if k not in ("id", "pid", "ip")},
    )
    db.add(host)
    db.flush()
    return host, True
...
def upsert_host_by_ip(
    ...
    *,
    ...
    if not _has_index(db, "hosts", "uq_hosts_pid_ip"):
        ...
        return _upsert_host_fallback(db, pid, ip, defaults, update_on_conflict)

    # PostgreSQL path — atomic insert with ON CONFLICT
    insert_values = {"id": defaults.get("id") or new_id("hst"), "pid": pid, "ip": ip}
    insert_values.update({k: v for k, v in defaults.items() if k not in insert_values})
