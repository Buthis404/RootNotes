...
from ..core.permissions import PERM_NETWORK_UPDATE
...
@router.get("", response_model=list[schemas.Network], responses={404: {"description": "Not found"}})
def list_networks(
...
@router.post("", response_model=schemas.Network, status_code=201, responses={404: {"description": "Not found"}})
def create_network(
    body: schemas.NetworkCreate,
    db: Annotated[Session, Depends(get_db)],
    user: Annotated[models.User, Depends(get_current_user)],
):
    check_pid_access(db, body.pid, user, PERM_NETWORK_UPDATE)
    net = models.Network(
        id=new_id("net"),
        pid=body.pid,
        name=body.name,
...
@router.patch("/{nid}", response_model=schemas.Network, responses={404: {"description": "Not found"}})
def update_network(
    nid: str,
    body: schemas.NetworkUpdate,
    db: Annotated[Session, Depends(get_db)],
    user: Annotated[models.User, Depends(get_current_user)],
):
    ...
    check_object_access(db, net.pid, user, PERM_NETWORK_UPDATE)
