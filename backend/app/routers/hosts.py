...
@router.get("", response_model=list[schemas.Host], responses={400: {"description": "Bad request"}, 404: {"description": "Not found"}})
def list_hosts(
    response: Response,
...
@router.post("", response_model=schemas.Host, status_code=201, responses={400: {"description": "Bad request"}, 404: {"description": "Not found"}})
def create_host(
    body: schemas.HostCreate,
    request: Request,
    db: Annotated[Session, Depends(get_db)],
    user: Annotated[models.User, Depends(get_current_user)],
):
...
@router.patch("/{hid}", response_model=schemas.Host, responses={400: {"description": "Bad request"}, 404: {"description": "Not found"}})
def update_host(
    hid: str,
    body: schemas.HostUpdate,
    request: Request,
    db: Annotated[Session, Depends(get_db)],
    user: Annotated[models.User, Depends(get_current_user)],
):
    ...
    for payload in node_payloads:
        bcast(
            ...
            "network",
            "node_updated",
            {"network_id": payload.pop("network_id", ""), "node": payload},
        )
    return host


@router.delete("/{hid}", status_code=204, responses={400: {"description": "Bad request"}, 404: {"description": "Not found"}})
def delete_host(
    hid: str,
    request: Request,
    db: Annotated[Session, Depends(get_db)],
...
def _try_add_plain_ip(token: str, ips: list, seen: set) -> None:
    try:
...
def _expand_ips(text: str) -> list[str]:
    ...
    for token in re.split(r"[\s,;]+", text):
        ...
        try:
            ...
            _try_add_plain_ip(token, ips, seen)
    return ips


@router.post("/bulk", status_code=201, responses={400: {"description": "Bad request"}, 404: {"description": "Not found"}})
def bulk_import_hosts(
    body: BulkHostImportBody,
    request: Request,
    db: Annotated[Session, Depends(get_db)],
    user: Annotated[models.User, Depends(get_current_user)],
):
    ...
    if created:
        log_event(
            ...
            "host",
            "bulk_import",
            f"Bulk import: {len(created)} hosts added",
            {"count": len(created)},
        )
    db.commit()
