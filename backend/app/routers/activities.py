...
router = APIRouter(
    prefix="/api/host-activities", tags=["host-activities"],
    responses={
...
def create_host_activity(
    body: schemas.HostActivityCreate,
    request: Request,
    db: Annotated[Session, Depends(get_db)],
    user: Annotated[models.User, Depends(get_current_user)],
):
    ...
    db.refresh(item)
    bcast(
        item.pid, "host_activity", "create", schemas.HostActivity.model_validate(item).model_dump()
    )
    return item


@router.patch("/{aid}", response_model=schemas.HostActivity, responses={404: {"description": "Not found"}})
def update_host_activity(
    aid: str,
    body: schemas.HostActivityUpdate,
    request: Request,
    db: Annotated[Session, Depends(get_db)],
    user: Annotated[models.User, Depends(get_current_user)],
):
    ...
    db.refresh(item)
    bcast(
        item.pid, "host_activity", "update", schemas.HostActivity.model_validate(item).model_dump()
    )
    return item


@router.delete("/{aid}", status_code=204, responses={404: {"description": "Not found"}})
def delete_host_activity(
    aid: str,
    request: Request,
    db: Annotated[Session, Depends(get_db)],
    user: Annotated[models.User, Depends(get_current_user)],
