...
router = APIRouter(
    prefix="/api/projects", tags=["members"],
    responses={
...
_MSG_OWNER_ONLY = "Only owners can assign the owner role"
...
def _upsert_member(
    pid: str, user_id: str, role: str, actor_id: str, db: Session
...
@router.get("/{pid}/members", response_model=list[MemberOut], responses={400: {"description": "Bad request"}, 403: {"description": "Forbidden"}, 404: {"description": "Not found"}})
def list_members(
    pid: str,
    ...
    members = (
        ...
        .filter(models.ProjectMember.project_id == pid, models.ProjectMember.is_active)
        .all()
    )
    return [
        MemberOut(
...
@router.post("/{pid}/members", status_code=201, response_model=MemberOut, responses={400: {"description": "Bad request"}, 403: {"description": "Forbidden"}, 404: {"description": "Not found"}})
def add_member(
    pid: str,
    body: AddMemberBody,
    db: Annotated[Session, Depends(get_db)],
    ...
    if body.role == MemberRole.OWNER and not is_admin(user):
        caller_membership = get_membership(db, pid, user.id)
        if not caller_membership or caller_membership.role != "owner":
            raise HTTPException(403, _MSG_OWNER_ONLY)

    target_user = (
        db.query(models.User).filter(models.User.id == body.user_id, models.User.active).first()
    )
    if not target_user:
...
@router.post("/{pid}/members/bulk", status_code=201, response_model=list[MemberOut], responses={400: {"description": "Bad request"}, 403: {"description": "Forbidden"}, 404: {"description": "Not found"}})
def bulk_add_members(
    ...
    target_users = (
        db.query(models.User).filter(models.User.id.in_(user_ids), models.User.active).all()
    )
...
@router.get("/{pid}/available-users", response_model=list[AvailableUserOut], responses={400: {"description": "Bad request"}, 403: {"description": "Forbidden"}, 404: {"description": "Not found"}})
def list_available_users(
    ...
    query = db.query(models.User).filter(models.User.active)
    if member_ids:
        query = query.filter(~models.User.id.in_(member_ids))

    users = query.order_by(models.User.username).all()
    return [
        AvailableUserOut(id=u.id, username=u.username, role=u.role, active=u.active) for u in users
    ]


@router.patch("/{pid}/members/{target_uid}", response_model=MemberOut, responses={400: {"description": "Bad request"}, 403: {"description": "Forbidden"}, 404: {"description": "Not found"}})
def update_member_role(
    ...
    if target_membership.role == MemberRole.OWNER and not is_admin(user):
        caller_membership = get_membership(db, pid, user.id)
        if not caller_membership or caller_membership.role != "owner":
    ...
    return MemberOut(
        ...
        username=target_user.username if target_user else target_uid,
        role=target_membership.role,
        created_at=target_membership.created_at,
        created_by=target_membership.created_by,
        is_active=target_membership.is_active,
    )


@router.delete("/{pid}/members/{target_uid}", status_code=204, responses={400: {"description": "Bad request"}, 403: {"description": "Forbidden"}, 404: {"description": "Not found"}})
def remove_member(
    pid: str,
    target_uid: str,
    db: Annotated[Session, Depends(get_db)],
...
@router.post("/{pid}/transfer-ownership", status_code=200, responses={400: {"description": "Bad request"}, 403: {"description": "Forbidden"}, 404: {"description": "Not found"}})
def transfer_ownership(
    ...
    if not is_admin(user):
        ...
        if caller_membership and caller_membership.user_id != body.user_id:
            caller_membership.role = MemberRole.ADMIN

    db.commit()
    return {"message": "Ownership transferred successfully"}


@router.get("/{pid}/permissions/me", responses={400: {"description": "Bad request"}, 403: {"description": "Forbidden"}, 404: {"description": "Not found"}})
