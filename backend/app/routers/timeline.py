...
router = APIRouter(
    prefix="/api/timeline", tags=["timeline"],
    responses={
...
@router.get("", response_model=list[schemas.TimelineEvent], responses={400: {"description": "Bad request"}, 404: {"description": "Not found"}})
def get_timeline(
    pid: str,
    response: Response,
    ...
    return (
        q.order_by(models.TimelineEvent.ts.desc())
        .offset(max(0, offset))
...
def _apply_undo_delete(db: Session, pid: str, entity: str, target_id: str, model_cls) -> dict:
...
def _apply_undo_patch(db: Session, pid: str, entity: str, target_id: str, model_cls, op: dict) -> dict:
...
def _apply_undo_op(db: Session, pid: str, op: dict) -> dict:
    ...
    if op_type == "delete":
        return _apply_undo_delete(db, pid, entity, target_id, model_cls)
    if op_type == "patch":
        return _apply_undo_patch(db, pid, entity, target_id, model_cls, op)
    raise HTTPException(400, f"Unsupported undo op type: {op_type!r}")


def _validated_batch_ops(undo: dict) -> list:
...
@router.post("/{event_id}/undo", responses={400: {"description": "Bad request"}, 404: {"description": "Not found"}})
def undo_event(
    event_id: str,
    db: Annotated[Session, Depends(get_db)],
    user: Annotated[models.User, Depends(get_current_user)],
    ...
    if undo_type in ("patch", "delete"):
        ...
        applied = [_apply_undo_op(db, event.pid, op) for op in _validated_batch_ops(undo)]
    else:
