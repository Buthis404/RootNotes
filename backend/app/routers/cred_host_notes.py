...
from ..core.permissions import PERM_CREDENTIALS_UPDATE
...
@router.get("", response_model=list[schemas.CredHostNote], responses={404: {"description": "Not found"}})
...
@router.post("", response_model=schemas.CredHostNote, status_code=201, responses={404: {"description": "Not found"}})
def create_cred_host_note(
    body: schemas.CredHostNoteCreate,
    db: Annotated[Session, Depends(get_db)],
    user: Annotated[models.User, Depends(get_current_user)],
):
    check_pid_access(db, body.pid, user, PERM_CREDENTIALS_UPDATE)
    existing = (
...
@router.patch("/{nid}", response_model=schemas.CredHostNote, responses={404: {"description": "Not found"}})
def update_cred_host_note(
    nid: str,
    body: schemas.CredHostNoteUpdate,
    db: Annotated[Session, Depends(get_db)],
    user: Annotated[models.User, Depends(get_current_user)],
):
    ...
    check_object_access(db, note.pid, user, PERM_CREDENTIALS_UPDATE)
...
@router.delete("/{nid}", status_code=204, responses={404: {"description": "Not found"}})
def delete_cred_host_note(
    nid: str,
    db: Annotated[Session, Depends(get_db)],
    user: Annotated[models.User, Depends(get_current_user)],
):
