...
│  │   • /data/uploads/  (loot files: plaintext on FS)        │   │
...
| Audit log | blocked | blocked | per-project audit events visible if role allows timeline read | **read** | **read everywhere** | **read** + tamper |
...
- **Forensic-grade tamper detection on the audit log.** The
  `timeline_events` table is mutable by a DB admin. Forward to a
  WORM-style log sink if you need stronger guarantees.
