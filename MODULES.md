...
`backend/app/plugins/modules/` and a JS module that calls `moduleRegistry.register()`
on the frontend side. Backend modules are auto-discovered at startup; frontend
modules must be imported by the frontend bundle.
...
frontend/src/features/plugins/ ← registry + extension-point helpers
frontend/src/...               ← import your module file from the active frontend bundle
```

Backend modules are discovered via `pkgutil.iter_modules` on startup.
Frontend modules must be explicitly imported (no browser-side auto-discovery).
