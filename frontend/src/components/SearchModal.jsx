...
const FILTER_KEYS = new Set(['type', 'severity', 'status', 'service', 'role', 'source', 'connector', 'tag']);

function parseTokens(raw) {
  const filters = {};
  const words = [];
  for (const tok of raw.trim().split(/\s+/)) {
      const m = /^(\w+):(\S+)$/.exec(tok);
      if (m && FILTER_KEYS.has(m[1])) {
      filters[m[1]] = m[2];
    } else if (tok) {
      words.push(tok);
    }
...
function FilterChip({ k, v, accent, onRemove }) {
  return (
    <span style={{
      ...
      <button type="button" onClick={onRemove} style={{ cursor: 'pointer', opacity: 0.6, marginLeft: 2, background: 'transparent', border: 'none', color: 'inherit', padding: 0 }}>×</button>
    </span>
  );
}
    ...
    <div style={{ borderBottom: '1px solid #13151c' }}>
      <button
        ...
        style={{
          ...
          cursor: 'pointer',
          background: active ? '#ffffff0c' : 'transparent', transition: 'background .08s', width: '100%', textAlign: 'left', border: 'none'
        }}
      ...
      {hasRelated && (
        <div style={{ paddingBottom: 4 }}>
          {item.related.map((r) => <RelatedItem key={`${r.type}-${r.title}`} rel={r} />)}
        </div>
      )}
    </div>
  );
...
ResultRow.propTypes = {
  ...
  projName: PropTypes.func,
  showPid: PropTypes.bool,
};
...
export default function SearchModal({ accent, selectedProject, projects, onNavigate, onClose }) {
  ...
  return (
    ...
    >
      <button type="button" aria-label="Close search" onClick={onClose} style={{ position: 'absolute', inset: 0, background: 'transparent', border: 'none', cursor: 'default' }} />
        ...
        {results && total > 0 && (
          <div style={{ display: 'flex', gap: 12, padding: '6px 18px', borderBottom: '1px solid #13151c', flexShrink: 0 }}>
            {Object.entries(facets).map(([t, n]) => {
              ...
              return (
                <button key={t} onClick={() => setQuery(q => _setTypeQuery(q, t))} style={{ background: 'transparent', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4, padding: 0 }}>
                  ...
                  {savedSearches.map(ss => (
                    <button
                      ...
                      style={{
                        ...
                        background: 'transparent', transition: 'background .08s', width: '100%', textAlign: 'left'
                      }}
                    ...
                    >
                      ...
                      <button
                        ...
                        style={{ fontSize: 12, color: '#353840', cursor: 'pointer', flexShrink: 0, padding: '0 2px', background: 'transparent', border: 'none' }}
                        title="Delete"
                      >×</button>
                    </button>
                  ))}
