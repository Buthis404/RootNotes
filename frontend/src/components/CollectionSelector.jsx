...
function FilterEditor({ value, onChange, accent }) {
  ...
  const chip = (val, arr, key) => (
    <button type="button" key={val} onClick={() => set(key, arr.filter(x => x !== val))}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
        ...
        <div>
          ...
          <input value={f.subnet || ''} onChange={e => set('subnet', e.target.value)} placeholder="x.x.x.x/24" style={inp} />
        </div>
      </div>
...
export default function CollectionSelector({ pid, accent, onSelect, selectedIds = [], onClear }) {
  ...
  const saveActionLabel = editingId ? 'Update' : 'Save';
