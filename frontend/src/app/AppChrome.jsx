...
function _triState(active, hov, activeVal, hoverVal, defaultVal) {
  if (active) return activeVal;
  if (hov) return hoverVal;
  return defaultVal;
}

export function NavTab({ tab, active, onClick, accent, badge, expanded }) {
  const [hov, setHov] = useState(false);
  const bgColor = _triState(active, hov, `${accent}18`, '#ffffff08', 'transparent');
  const iconColor = _triState(active, hov, accent, '#9098a8', '#404550');
  const labelColor = _triState(active, hov, accent, '#9098a8', '#606570');
  return (
    <button onClick={onClick} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)} title={tab.label}
      style={{ width: '100%', padding: '12px 14px', border: 'none', cursor: 'pointer', background: bgColor, borderLeft: active ? `2px solid ${accent}` : '2px solid transparent', display: 'flex', flexDirection: expanded ? 'row' : 'column', alignItems: 'center', justifyContent: expanded ? 'flex-start' : 'center', gap: 10, transition: 'all .15s', position: 'relative' }}>
      <Icon name={tab.icon} size={20} color={iconColor} />
      {expanded && <span style={{ fontSize: 10, color: labelColor, letterSpacing: '0.04em', textTransform: 'uppercase', fontWeight: 600, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{tab.label}</span>}
...
NavTab.propTypes = { tab: PropTypes.any, active: PropTypes.any, onClick: PropTypes.any, accent: PropTypes.any, badge: PropTypes.any, expanded: PropTypes.any };
      ...
      {projects.map(p => {
        ...
        return (
          <button type="button" key={p.id} onClick={() => onSelect(p.id)}
            style={{ padding: '10px 14px', cursor: 'pointer', background: act ? `${accent}18` : 'transparent', borderLeft: act ? `2px solid ${accent}` : '2px solid transparent', transition: 'all .12s', borderTop: 'none', borderRight: 'none', borderBottom: 'none', width: '100%', textAlign: 'left' }}
...
ProjectPicker.propTypes = { projects: PropTypes.any, hosts: PropTypes.any, selected: PropTypes.any, onSelect: PropTypes.any, accent: PropTypes.any };
...
TweaksPanel.propTypes = { tweaks: PropTypes.any, updateTweak: PropTypes.any, onClose: PropTypes.any, left: PropTypes.any };
