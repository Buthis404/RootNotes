...
ConfirmDialog.propTypes = { text: PropTypes.string, onConfirm: PropTypes.func, onCancel: PropTypes.func };
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              {['viewer', 'user', 'admin'].map(r => (
                <button key={r} onClick={() => setRole(r)}
                  ...
                  {{ admin: 'Administrator', viewer: 'Viewer' }[r] || 'User'}
                </button>
              ))}
            </div>
          </div>
...
CreateUserModal.propTypes = { accent: PropTypes.string, onClose: PropTypes.func, onCreated: PropTypes.func };
...
ResetPasswordModal.propTypes = { user: PropTypes.object, accent: PropTypes.string, onClose: PropTypes.func };
...
ProjectAccessSection.propTypes = { accent: PropTypes.string, allUsers: PropTypes.array };
...
function AISection({ accent }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      globalThis.dispatchEvent(new Event('rt:ai_status_changed'));
      setMsg('Saved.');
  ...
  const handleProviderChange = (val) => {
    ...
    setNewBaseUrl(PROVIDER_BASE_URLS[val] || '');
  };
  ...
  const [editSkipTools, setEditSkipTools] = useState(false);

  const startEdit = (idx) => {
    const p = config.providers[idx];
    setEditIdx(idx);
    setEditName(p.name || '');
    setEditProvider(p.provider || 'openai');
    setEditApiKey(p.api_key || '');
    setEditModel(p.model || '');
    setEditBaseUrl(p.base_url || '');
    setEditPriority(p.priority || 1);
    setEditSkipTools(!!p.skip_tools);
  };

  const commitEdit = () => {
    setConfig(prev => ({
      ...prev,
      providers: prev.providers.map((p, i) => i === editIdx ? {
        ...p,
        name: editName.trim() || p.name,
        provider: editProvider,
        api_key: editApiKey || p.api_key,
        model: editModel,
        base_url: editBaseUrl,
        priority: Number.parseInt(editPriority) || 1,
        skip_tools: editSkipTools,
      } : p),
    }));
    setEditIdx(-1);
  };
  ...
  const addProvider = () => {
    ...
    const p = {
      ...
      model: newModel,
      base_url: newBaseUrl,
      priority: Number.parseInt(newPriority) || 1,
      enabled: true,
    };
      ...
      {!loading && (
        <div style={sectionBox}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
                ...
                <div>
                  ...
                  <select value={newProvider} onChange={e => handleProviderChange(e.target.value)} style={{ ...inp(), cursor: 'pointer' }}>
                    {['anthropic', 'openai', 'ollama', 'groq', 'mistral', 'openrouter', 'litellm'].map(p => (
                ...
                <div>
                  <div style={label}>Base URL (optional override)</div>
                  <input value={newBaseUrl} onChange={e => setNewBaseUrl(e.target.value)} placeholder={PROVIDER_BASE_URLS[newProvider] || 'http://localhost:4000'} style={inp()} />
                </div>
                <div>
          ...
          {config.providers.map((p, i) => (
            <div key={p.name + '-' + i} style={{ padding: '9px 0', borderBottom: i < config.providers.length - 1 ? '1px solid #1e2029' : 'none' }}>
              {editIdx === i ? (
                <div style={{ background: '#090b0f', border: '1px solid #2a2d35', borderRadius: 6, padding: '14px' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
                    <div>
                    ...
                    <div>
                      <div style={label}>Provider</div>
                      <select value={editProvider} onChange={e => { setEditProvider(e.target.value); setEditModel(PROVIDER_DEFAULT_MODELS[e.target.value] || editModel); }} style={{ ...inp(), cursor: 'pointer' }}>
                        {['anthropic', 'openai', 'ollama', 'groq', 'mistral', 'openrouter', 'litellm'].map(pr => (
                    ...
                    <div>
                      <div style={label}>API Key</div>
                      <input type="password" value={editApiKey} onChange={e => setEditApiKey(e.target.value)} placeholder="(unchanged)" style={inp()} />
                    </div>
                    <div>
                      <div style={label}>Model</div>
                      <input value={editModel} onChange={e => setEditModel(e.target.value)} style={inp()} />
                    </div>
                    <div>
                      <div style={label}>Base URL</div>
                      <input value={editBaseUrl} onChange={e => setEditBaseUrl(e.target.value)} style={inp()} />
                    </div>
                    <div>
                      <div style={label}>Priority</div>
                      <input type="number" value={editPriority} onChange={e => setEditPriority(e.target.value)} min={1} style={inp()} />
                    </div>
                  </div>
                  <label style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12, cursor: 'pointer', fontSize: 11, color: editSkipTools ? accent : '#606570', fontFamily: 'JetBrains Mono' }}>
                    <input type="checkbox" checked={editSkipTools} onChange={e => setEditSkipTools(e.target.checked)} style={{ accentColor: accent }} />
                    {" "}Skip tools (thinking / reasoning models that don&apos;t support function calling)
                  </label>
                  <div style={{ display: 'flex', gap: 8 }}>
                    <button onClick={commitEdit} style={{ background: accent, border: 'none', borderRadius: 4, padding: '5px 14px', cursor: 'pointer', color: '#fff', fontSize: 11, fontFamily: 'JetBrains Mono', fontWeight: 600 }}>Save</button>
                    <button onClick={() => setEditIdx(-1)} style={{ background: 'transparent', border: '1px solid #2a2d35', borderRadius: 4, padding: '5px 14px', cursor: 'pointer', color: '#606570', fontSize: 11, fontFamily: 'JetBrains Mono' }}>Cancel</button>
                  </div>
                </div>
              ) : (
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 10, color: '#606570', fontFamily: 'JetBrains Mono' }}>
                      ...
                      {p.skip_tools && <span style={{ marginLeft: 6, color: '#c88200' }}>· no-tools</span>}
                    </div>
                  </div>
                  <button onClick={() => startEdit(i)} style={{ background: 'transparent', border: '1px solid #2a2d35', borderRadius: 4, padding: '3px 8px', cursor: 'pointer', color: '#606570', fontSize: 10, fontFamily: 'JetBrains Mono' }}
                    onMouseEnter={e => { e.currentTarget.style.borderColor = accent; e.currentTarget.style.color = accent; }}
                    onMouseLeave={e => { e.currentTarget.style.borderColor = '#2a2d35'; e.currentTarget.style.color = '#606570'; }}>Edit</button>
        ...
        <div style={sectionBox}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <div>
              ...
              <input type="number" value={config.max_tool_calls} min={1} max={100} onChange={e => setConfig(prev => ({ ...prev, max_tool_calls: Number.parseInt(e.target.value) || 10 }))} style={{ ...inp(), width: 120 }} />
            </div>
          </div>
        </div>
      )}
...
AISection.propTypes = { accent: PropTypes.string };
          ...
          <div>
            ...
            {detectedChats.length > 0 && (
              <div style={{ marginTop: 6, background: '#0a0c10', border: '1px solid #2a2d35', borderRadius: 4, overflow: 'hidden' }}>
                {detectedChats.map(c => (
                  <button type="button" key={c.id} onClick={() => { set('telegram.chat_id', c.id); setDetectedChats([]); }}
                    style={{ padding: '6px 10px', cursor: 'pointer', borderBottom: '1px solid #1a1c22', fontSize: 11, color: '#c8cdd6', fontFamily: 'JetBrains Mono', width: '100%', textAlign: 'left', background: 'transparent', borderTop: 'none', borderLeft: 'none', borderRight: 'none' }}
...
NotificationsSection.propTypes = { accent: PropTypes.string };
          ...
          <div key={u.id} style={{ display: 'grid', gridTemplateColumns: tableColumns, gap: 0, padding: '13px 18px', borderBottom: i < users.length - 1 ? '1px solid #14161b' : 'none', alignItems: 'center', background: isSelf(u) ? `${accent}08` : 'transparent' }}>
            ...
            <div>
              <span style={{ fontSize: 10, fontFamily: 'JetBrains Mono', fontWeight: 600, color: GLOBAL_ROLE_COLOR[u.role], background: `${GLOBAL_ROLE_COLOR[u.role]}18`, border: `1px solid ${GLOBAL_ROLE_COLOR[u.role]}44`, borderRadius: 4, padding: '2px 8px', textTransform: 'uppercase' }}>
                {{ admin: 'Admin', viewer: 'Viewer' }[u.role] || 'User'}
              </span>
            </div>
...
AdminView.propTypes = { currentUser: PropTypes.object, accent: PropTypes.string, onlineUsers: PropTypes.array };
