  ...
  return (
    <div style={{ flex: 1, display: 'flex', overflow: 'hidden', background: '#090b0f' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        {/* Toolbar */}
        <div style={{ padding: '10px 20px', borderBottom: '1px solid #1e2029', display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
          <div style={{ flex: 1, fontSize: 12, color: '#606570', fontFamily: 'JetBrains Mono' }}>
            {filtered.length} article{filtered.length !== 1 ? 's' : ''}
            {selectedCat ? ` in ${selectedCat}` : ''}
          </div>
          <button
            onClick={() => { setShowCreate(v => !v); setSelectedArticle(null); setEditing(false); }}
            style={{ background: accent, border: 'none', borderRadius: 5, padding: '6px 14px', cursor: 'pointer', color: '#fff', fontSize: 11, fontWeight: 600, fontFamily: 'JetBrains Mono' }}
          >
            + New Article
          </button>
        </div>
        ...
        {/* Error */}
        {error && <div style={{ margin: 16, background: '#cc233318', border: '1px solid #cc233344', borderRadius: 5, padding: '8px 12px', fontSize: 11, color: '#cc2233', fontFamily: 'JetBrains Mono' }}>{error}</div>}

        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px' }}>
          {/* Create form */}
          {showCreate && (
            <div style={{ ...sectionBox, padding: '16px 18px', marginBottom: 16 }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: '#f0f2f6', fontFamily: 'Space Grotesk', marginBottom: 14 }}>New Article</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                <div>
                  <div style={{ fontSize: 9, color: '#404550', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 4 }}>Title</div>
                  <input value={newTitle} onChange={e => setNewTitle(e.target.value)} placeholder="Article title" style={inp()} />
                </div>
                ...
                <div>
                  <div style={{ fontSize: 9, color: '#404550', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 4 }}>Content</div>
                  <textarea value={newContent} onChange={e => setNewContent(e.target.value)} placeholder="Article content (markdown or plain text)" rows={15} style={{ ...inp(), resize: 'vertical', lineHeight: 1.6 }} />
                </div>
                <div style={{ display: 'flex', gap: 8 }}>
          ...
          {selectedArticle && !showCreate && (
            <div style={{ ...sectionBox, padding: '18px 20px', marginBottom: 16 }}>
              {editing ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  ...
                  <div>
                    <div style={{ fontSize: 9, color: '#404550', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 4 }}>Content</div>
                    <textarea value={editContent} onChange={e => setEditContent(e.target.value)} rows={15} style={{ ...inp(), resize: 'vertical', lineHeight: 1.6 }} />
                  </div>
                  <div style={{ display: 'flex', gap: 8 }}>
                ...
                <>
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginBottom: 14 }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 18, fontWeight: 700, color: '#f0f2f6', fontFamily: 'Space Grotesk', marginBottom: 8 }}>{selectedArticle.title}</div>
                    ...
                    <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                      ...
                      <button onClick={() => setSelectedArticle(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#606570', fontSize: 16 }}>✕</button>
                    </div>
                  </div>
                  {selectedArticle.created_at && (
                    <div style={{ fontSize: 10, color: '#404550', fontFamily: 'JetBrains Mono', marginBottom: 14 }}>
                      Created: {new Date(selectedArticle.created_at).toLocaleDateString()}
                    </div>
                  )}
                  {/* Content */}
                  <pre style={{ whiteSpace: 'pre-wrap', wordBreak: 'break-word', fontFamily: 'JetBrains Mono', fontSize: 12, color: '#c8cdd6', lineHeight: 1.7, margin: 0, background: '#090b0f', border: '1px solid #1e2029', borderRadius: 6, padding: '14px 16px' }}>
              ...
              {confirmDelete && (
                <div style={{ marginTop: 14, background: '#cc233318', border: '1px solid #cc233344', borderRadius: 6, padding: '12px 14px' }}>
                  <div style={{ fontSize: 12, color: '#f0f2f6', marginBottom: 10 }}>Delete article "{selectedArticle.title}"? This cannot be undone.</div>
                  <div style={{ display: 'flex', gap: 8 }}>
          ...
          {!selectedArticle && !showCreate && (
            <>
              {loading && <div style={{ color: '#404550', fontSize: 12, fontFamily: 'JetBrains Mono', textAlign: 'center', padding: 40 }}>Loading…</div>}
              {!loading && filtered.length === 0 && (
                <div style={{ color: '#404550', fontSize: 12, fontFamily: 'JetBrains Mono', textAlign: 'center', padding: 40 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {filtered.map(art => (
                  <div
                    key={art.id}
                    onClick={() => { setSelectedArticle(art); setEditing(false); setConfirmDelete(false); }}
                    style={{ ...sectionBox, padding: '12px 16px', cursor: 'pointer', transition: 'border-color .1s' }}
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                      <div style={{ flex: 1, fontSize: 13, fontWeight: 600, color: '#e0e4ec', fontFamily: 'Space Grotesk' }}>{art.title}</div>
                      <span style={{ fontSize: 9, background: art.pid ? accent + '22' : '#c07af022', border: `1px solid ${art.pid ? accent + '44' : '#c07af044'}`, borderRadius: 3, padding: '2px 6px', color: art.pid ? accent : '#c07af0', fontFamily: 'JetBrains Mono', flexShrink: 0 }}>
                        {art.pid ? 'PROJECT' : 'GLOBAL'}
                      </span>
                    </div>
                    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
                      <span style={{ fontSize: 9, background: categoryColor(art.category) + '18', border: `1px solid ${categoryColor(art.category)}33`, borderRadius: 3, padding: '1px 6px', color: categoryColor(art.category), fontFamily: 'JetBrains Mono' }}>
                        {art.category}
                      </span>
                      {(art.tags || []).map(t => (
                        <span key={t} style={{ fontSize: 9, background: '#1a1c22', border: '1px solid #2a2d35', borderRadius: 3, padding: '1px 5px', color: '#606570', fontFamily: 'JetBrains Mono' }}>{t}</span>
                      ))}
                      {art.created_at && (
                        <span style={{ marginLeft: 'auto', fontSize: 9, color: '#404550', fontFamily: 'JetBrains Mono' }}>
