              ...
              <div key={scope.id}
                ...
                {isEditing ? (
                  ...
                  <>
                    ...
                    <span style={{ fontSize: 10, color: '#808590', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{scope.description || '—'}{scope.gateway_ip ? ` · gw ${scope.gateway_ip}` : ''}{scope.is_entry ? ' · entry' : ''}{scope.via_host_id ? ` · via ${projectHosts.find(h => h.id === scope.via_host_id)?.hostname || projectHosts.find(h => h.id === scope.via_host_id)?.ip || scope.via_host_id}` : ''}</span>
                    <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexShrink: 0 }}>
                      {[[true, '✓ In Scope', '#39d353'], [false, '✗ Exclude', '#cc2233']].map(([v, l, c]) => (
