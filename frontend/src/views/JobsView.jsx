...
function _fcSeverityColor(sev) {
  if (sev === 'critical') return '#f87171';
  if (sev === 'high') return '#f09a3a';
  return '#e0a820';
}

function _groupStatusFromCounts(counts) {
...
TypeBadge.propTypes = { type: PropTypes.any };
...
MetaBadge.propTypes = { value: PropTypes.any, color: PropTypes.any };
...
StructuredBadges.propTypes = { result: PropTypes.any };
...
function LiveOutputPanel({ pid, jobId, accent, onClose }) {
  ...
  const statusColor = (() => {
    if (finalStatus === 'done') return '#39d353';
    if (finalStatus === 'failed') return '#cc2233';
    return '#f09a3a';
  })();
  ...
  return (
    <div style={{ position: 'fixed', bottom: 20, right: 20, width: 640, maxHeight: '60vh', background: '#070a10', border: `1px solid ${accent}44`, borderRadius: 8, boxShadow: '0 8px 40px #00000099', zIndex: 300, display: 'flex', flexDirection: 'column' }}>
      ...
      <pre style={{ flex: 1, overflowY: 'auto', margin: 0, padding: '10px 12px', fontSize: 11, color: '#c8cfe0', fontFamily: 'JetBrains Mono', lineHeight: 1.5, background: 'transparent' }}>
        ...
        {lines.map((l, i) => <span key={`line-${i}-${l.substring(0, 20)}`} style={{ display: 'block' }}>{l}</span>)}
        <span ref={bottomRef} />
      </pre>
...
LiveOutputPanel.propTypes = { pid: PropTypes.any, jobId: PropTypes.any, accent: PropTypes.any, onClose: PropTypes.any };
      ...
      {enrichment.host_changes?.length > 0 && (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
          {enrichment.host_changes.map((ch, i) => (
            <span key={`hc-${ch.field}-${i}`} style={{ fontSize: 10, fontFamily: 'JetBrains Mono', color: '#808590', background: '#141620', borderRadius: 3, padding: '2px 6px' }}>
      ...
      {enrichment.new_creds?.length > 0 && (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginTop: enrichment.host_changes?.length ? 4 : 0 }}>
          {enrichment.new_creds.map((c, i) => (
            <span key={`cred-${c.username}-${c.type}-${i}`} style={{ fontSize: 10, fontFamily: 'JetBrains Mono', color: '#c07af0', background: '#141620', borderRadius: 3, padding: '2px 6px' }}>
...
EnrichmentPanel.propTypes = { enrichment: PropTypes.any };
...
function renderHighlightedOutput(text, query) {
  ...
  const rendered = lines.map((line, i) => {
    ...
    if (idx === -1) {
      return <span key={`dim-${i}-${line.substring(0, 10)}`} style={{ display: 'block', opacity: 0.3 }}>{line || ' '}</span>;
    }
    matchCount++;
    return (
      <span key={`hl-${i}-${line.substring(0, 10)}`} style={{ display: 'block', background: '#2a2000' }}>
        {line.slice(0, idx)}
  ...
  return (
    <div style={{ marginBottom: 8, padding: '6px 10px', background: '#0a1018', border: '1px solid #5b8af533', borderRadius: 4 }}>
      ...
      {artifacts.map((a, i) => {
        ...
        return (
          <div key={`art-${a.id || a.name || i}`} style={{ display: 'flex', alignItems: 'flex-start', gap: 8, marginBottom: 5, paddingBottom: 5, borderBottom: i < artifacts.length - 1 ? '1px solid #1a1c22' : 'none' }}>
...
ArtifactsPanel.propTypes = { pid: PropTypes.any, jobId: PropTypes.any, accent: PropTypes.any };
      ...
      >
        <td style={{ padding: '8px 10px', paddingLeft: indent ? 22 : 10 }}>
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 4 }}>
            ...
            {job.retry_count > 0 && (() => {
              const retryMax = job.max_retries > 0 ? `/${job.max_retries}` : '';
              return <MetaBadge value={`↻ retry ${job.retry_count}${retryMax}`} color="#c07af0" />;
            })()}
          </div>
            ...
            {job.result_json?.structured?.finding_candidates?.length > 0 && (
              <div style={{ marginBottom: 8, padding: '6px 10px', background: '#1a1500', border: '1px solid #e0a82033', borderRadius: 4 }}>
                ...
                {job.result_json.structured.finding_candidates.map((fc, i) => {
                  const fcColor = _fcSeverityColor(fc.severity);
                  return (
                  <div key={`fc-${i}-${fc.title}`} style={{ fontSize: 11, color: '#c8cdd6', display: 'flex', gap: 8, alignItems: 'center', marginBottom: 2 }}>
                    <span style={{ fontSize: 9, color: fcColor, background: fcColor + '18', border: `1px solid ${fcColor}33`, borderRadius: 3, padding: '1px 5px', fontFamily: 'JetBrains Mono' }}>{fc.severity}</span>
                    <span>{fc.title}</span>
...
JobRow.propTypes = { job: PropTypes.any, pid: PropTypes.any, accent: PropTypes.any, onCancel: PropTypes.any, onDelete: PropTypes.any, onRerun: PropTypes.any, onRetry: PropTypes.any, onLive: PropTypes.any, outputSearchQuery: PropTypes.any, indent: PropTypes.any };
...
function PlaybookRunGroup({ run, jobs, accent, pid, onCancel, onDelete, onRerun, onRetry, onLive, outputSearchQuery }) {
  ...
  const groupStatus = _groupStatusFromCounts(counts);
  ...
  return (
    <div style={{ marginBottom: 6, border: `1px solid ${cfg.color}28`, borderRadius: 6, overflow: 'hidden' }}>
      ...
      <button
        ...
        style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', background: `${cfg.color}0c`, cursor: 'pointer', userSelect: 'none', width: '100%', textAlign: 'left', border: 'none' }}
...
PlaybookRunGroup.propTypes = { run: PropTypes.any, jobs: PropTypes.any, accent: PropTypes.any, pid: PropTypes.any, onCancel: PropTypes.any, onDelete: PropTypes.any, onRerun: PropTypes.any, onRetry: PropTypes.any, onLive: PropTypes.any, outputSearchQuery: PropTypes.any };
...
export default function JobsView({ selectedProject, accent, jobs: jobsProp, onJobUpdate, onJobDelete, initialFilter, onFilterConsumed }) {
  ...
  // Auto-poll while running/queued jobs exist. When the WS is up, job
  // updates already arrive as `job:upsert` events — drop the cadence to
  // a slow fallback (30s) so we still catch anything the WS missed
  // without burning bandwidth on the happy path.
  ...
  useEffect(() => {
    ...
    const schedule = () => {
      clearInterval(pollRef.current);
      const interval = isWsConnected() ? 30000 : 3000;
    ...
    return () => {
      clearInterval(pollRef.current);
      globalThis.removeEventListener('rt:ws-state', onWs);
    };
  }, [hasActive, load]);
  ...
  // Same pattern as HostsView: defer the search keystrokes so a burst
  // of input only triggers one filter pass, and memoise the result so
  // unrelated state (poll tick, expand/collapse) doesn't refilter.
  ...
  }, [jobs, filter, typeFilter, connectorFilter, playbookRunFilter, deferredSearch]);
...
JobsView.propTypes = { selectedProject: PropTypes.any, accent: PropTypes.any, jobs: PropTypes.any, onJobUpdate: PropTypes.any, onJobDelete: PropTypes.any, initialFilter: PropTypes.any, onFilterConsumed: PropTypes.any };
  ...
  return (
    <div style={{ padding: '20px 24px', height: '100%', overflowY: 'auto' }}>
          ...
          <thead>
            <tr style={{ borderBottom: '1px solid #1e2230' }}>
              {['Job', 'Status', 'By', 'Created', 'Finished', 'Duration', ''].map((h, i) => (
                <th key={`th-${h || 'action'}`} style={{ padding: '6px 10px', color: '#6a7080', fontWeight: 500, fontSize: 11, textAlign: h === '' ? 'right' : 'left' }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
      ...
      {displayed.length > 0 && viewMode === 'grouped' && (() => {
        ...
        const _groupPriority = (jobs) => {
          if (jobs.some(j => j.status === 'running')) return 0;
          if (jobs.some(j => j.status === 'failed')) return 1;
          return 2;
        };
        const sortedGroups = [...grouped.entries()].sort(([, ajobs], [, bjobs]) => {
          const aPri = _groupPriority(ajobs);
          const bPri = _groupPriority(bjobs);
