...
| `viewer` | Legacy read-only account: limited to GET requests at the middleware level. **Not the same as the project-level `viewer` role** — this is a global system role enforced at HTTP middleware. Avoid assigning to new users; prefer granting `user` with a project-level `viewer` membership instead. |
